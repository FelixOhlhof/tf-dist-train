...
class Client():
    def __init__(self, hostname, port, client_id, client_count, dataset_name, worse_weight_flag):
        ...
        self.send_worse_weights = worse_weight_flag
        self.data_dir = util.copy_pictures(f"{Path.home()}\\.keras\\datasets\\{dataset_name}", self.client_id,
                                           self.client_count)
    ...
    def start_dist_training(self, epochs):
        ...
        for i in range(epochs):
            ...
            print('history: ', validation.history)
            ...
            if (validation.history['accuracy'][0] > self.accuracy) & self.send_worse_weights:
                self.accuracy = validation.history['accuracy']
...
if __name__ == "__main__":
    client = Client(config["SERVER"]["HOST"], (int)(config["SERVER"]["PORT"]), client_id=args.id,
                    client_count=(int)(config["CLIENT"]["TOTAL_CLIENTS"]),
                    dataset_name=config["CLIENT"]["DATASET_NAME"],
                    worse_weight_flag=config["CLIENT"]["SEND_WEIGHTS_WITHOUT_IMPROVEMENT"]
                    )
